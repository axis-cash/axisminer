/*
    This file is part of axisminer.

    axisminer is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    axisminer is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with axisminer.  If not, see <http://www.gnu.org/licenses/>.
*/

#include <CLI/CLI.hpp>

#include <axisminer/buildinfo.h>
...
bool g_exitOnError = false;  // Whether or not axisminer should exit on mining threads errors
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("axisminer - GPU progpow miner");
    ...
    {
        // clang-format off
        cout << "axisminer - GPU ethash miner" << endl
             << "minimal usage : axisminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default axisminer will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    axisminer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes axisminer can stay" << endl
                 ...
                 << endl
                 << "    --exit              FLAG Stop axisminer whenever an error is encountered" << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects axisminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    axisminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'axisminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When axisminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode axisminer tries to auto-detect the correct" << endl
                 ...
                 << "        stratum1    AXIS Proxy compatible" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = axisminer_get_buildinfo();
    cout << endl
         << endl
         << "axisminer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'axisminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try axisminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
